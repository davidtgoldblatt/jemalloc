...
static void
arena_edatas_lookup_size_check(tsd_t *tsd, cache_bin_ptr_array_t *arr,
    szind_t szind, size_t nflush, edata_t **edatas) {
...
arena_cache_bin_flush_impl(tsd_t *tsd, cache_bin_t *cache_bin,
    cache_bin_info_t *cache_bin_info, szind_t szind, unsigned rem,
    arena_t *stats_arena, bool small) {
	/*
	...
	cache_bin_init_ptr_array_for_flush(cache_bin, cache_bin_info, &ptrs,
	    nflush);

	/* Look up edata once per item. */
	if (config_opt_safety_checks) {
		arena_edatas_lookup_size_check(tsd, &ptrs, szind, nflush,
		    item_edata);
	} else {
		for (unsigned i = 0 ; i < nflush; i++) {
	...
	while (nflush > 0) {
		...
		if (small) {
			...
			cur_bin = &cur_arena->bins[szind].bin_shards[
			    cur_binshard];
			assert(cur_binshard < bin_infos[szind].n_shards);
		}
	...
	if (config_stats && !merged_stats) {
		if (small) {
			 ...
			 */
			unsigned binshard;
			bin_t *bin = arena_bin_choose_lock(tsdn, stats_arena,
			    szind, &binshard);
			bin->stats.nflushes++;
			bin->stats.nrequests += cache_bin->tstats.nrequests;
			cache_bin->tstats.nrequests = 0;
