...
arena_extent_alloc_large(tsdn_t *tsdn, arena_t *arena, size_t usize,
	...
	if (extent == NULL && arena_may_have_muzzy(arena)) {
		extent = extents_alloc(tsdn, arena, ehooks,
		    &arena->eset_muzzy, NULL, usize, sz_large_pad, alignment,
		    false, szind, zero, &commit);
	}
	size_t size = usize + sz_large_pad;
	if (extent == NULL) {
		extent = extent_alloc_wrapper(tsdn, arena, ehooks, NULL,
...
arena_stash_decayed(tsdn_t *tsdn, arena_t *arena,
	...
	while (nstashed < npages_decay_max &&
	    (extent = extents_evict(tsdn, arena, ehooks, eset,
	    npages_limit)) != NULL) {
...
arena_decay_stashed(tsdn_t *tsdn, arena_t *arena,
    ehooks_t *ehooks, arena_decay_t *decay, eset_t *eset,
	...
	for (extent_t *extent = extent_list_first(decay_extents); extent !=
		...
		case extent_state_muzzy:
			extent_dalloc_wrapper(tsdn, arena, ehooks,
			    extent);
			if (config_stats) {
				nunmapped += npages;
			}
...
arena_decay_to_limit(tsdn_t *tsdn, arena_t *arena, arena_decay_t *decay,
	...
	if (npurge != 0) {
		size_t npurged = arena_decay_stashed(tsdn, arena,
		    ehooks, decay, eset, all, &decay_extents,
		    is_background_thread);
...
extent_hooks_t *
arena_extent_hooks_set(tsd_t *tsd, arena_t *arena,
...
arena_dss_prec_get(arena_t *arena) {
	return (dss_prec_t)atomic_load_u(&arena_get_ehooks(arena)->dss_prec,
	    ATOMIC_ACQUIRE);
}
...
arena_dss_prec_set(arena_t *arena, dss_prec_t dss_prec) {
	...
	atomic_store_u(&arena_get_ehooks(arena)->dss_prec, (unsigned)dss_prec, ATOMIC_RELEASE);
	return false;
}
