...
pa_shard_stats_merge(tsdn_t *tsdn, pa_shard_t *shard,
	...
	*resident += (resident_pgs << LG_PAGE);
	...
	if (shard->ever_used_hpa) {
		...
		psset_bin_stats_accum(&hpa_stats_out->psset_fullstats,
		    &shard->hpa_shard.psset.fullstats);
		for (pszind_t i = 0; i < PSSET_NPSIZES; i++) {
			psset_bin_stats_accum(&hpa_stats_out->psset_stats[i],
			    &shard->hpa_shard.psset.stats[i]);
		}
