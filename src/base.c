...
base_map(tsdn_t *tsdn, ehooks_t *ehooks, unsigned ind, size_t size) {
	...
	if (ehooks_are_default(ehooks)) {
		...
		/*
		 * We use the raw reentrancy variant, since calling the
		 * pre-reentrancy hook with NULL may fail during bootstrapping.
		 */
 ...
 */
static base_block_t *
base_block_alloc(tsdn_t *tsdn, base_t *base, ehooks_t *ehooks,
...
base_new(tsdn_t *tsdn, unsigned ind, extent_hooks_t *extent_hooks) {
	/*
	 * The "real" ehooks will live in the arena, but we don't know where the
	 * arena will be until we allocate it.  We construct this one to use in
	 * the meantime.
	 */
	...
	memcpy(&base->ehooks, &fake_ehooks, sizeof(ehooks_t));
...
base_delete(tsdn_t *tsdn, base_t *base) {
	/* As in base_new. */
	ehooks_t fake_ehooks;
	memcpy(&fake_ehooks, base_ehooks_get(base), sizeof(ehooks_t));
	base_block_t *next = base->blocks;
	do {
		base_block_t *block = next;
		next = block->next;
		base_unmap(tsdn, &fake_ehooks, base_ind_get(base), block,
		    block->size);
	} while (next != NULL);
}
...
extent_hooks_t *base_extent_hooks_set(base_t *base,
	...
	extent_hooks_t *old = ehooks_user_ptr_get(&base->ehooks);
