...
stats_expect(psset_t *psset, size_t nactive) {
	if (nactive == PAGESLAB_PAGES) {
		expect_zu_eq(1, psset->fullstats.npageslabs,
		    "Expected a full slab");
		expect_zu_eq(PAGESLAB_PAGES, psset->fullstats.nactive,
		    "Should have exactly filled the bin");
		expect_zu_eq(0, psset->fullstats.ninactive,
		    "Should never have inactive pages in a full slab");
	} else {
		stats_expect_empty(&psset->fullstats);
	}
	...
	for (pszind_t i = 0; i < PSSET_NPSIZES; i++) {
		if (i == nonempty_pind) {
			assert_zu_eq(1, psset->stats[i].npageslabs,
			    "Should have found a slab");
			expect_zu_eq(nactive, psset->stats[i].nactive,
			    "Mismatch in active pages");
			expect_zu_eq(ninactive, psset->stats[i].ninactive,
			    "Mismatch in inactive pages");
		} else {
			stats_expect_empty(&psset->stats[i]);
		}
	}
}
