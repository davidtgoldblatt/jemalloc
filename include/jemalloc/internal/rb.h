...
#define rb_empty_summarize(a_node, a_child1, a_child2) false
...
a_attr a_type *								\
a_prefix##search(a_rbt_type *rbtree, const a_type *key, void *ctx);	\
a_attr a_type *								\
a_prefix##nsearch(a_rbt_type *rbtree, const a_type *key, void *ctx);	\
a_attr a_type *								\
a_prefix##psearch(a_rbt_type *rbtree, const a_type *key, void *ctx);	\
a_attr void								\
 ...
 *
 *     bool a_summarize(a_type *a_node, const a_type *a_child1,
 *         const a_type *a_child2);
 *
 *     This function should update a_node with the summary of the subtree rooted
 *     there, using the data contained in it and the summaries in a_child1 and
 *     a_child2.  One or both of them may be NULL.  When the tree changes due to
 *     an insertion or removal, it updates the summaries of all nodes whose
 *     subtrees have changed (always updating the summaries of children before
 *     their parents).  If the user alters a node in the tree in a way that
 *     may change its summary, they can call the generated update_summaries
 *     function to bubble up the summary changes to the root.  It should return
 *     true if the summary changed (or may have changed), and false if it
 *     didn't (which will allow the implementation to terminate "bubbling up"
 *     the summaries early).
 *
 ...
 * These extended accept three additional arguments:
 *
 ...
 *     if (child1 != NULL && child1->max_subtree_ffs > new_max_subtree_ffs) {
 *         new_max_subtree_ffs = child1->max_subtree_ffs;
 *     }
 *     if (child2 != NULL && child2->max_subtree_ffs > new_max_subtree_ffs) {
 *         new_max_subtree_ffs = child2->max_subtree_ffs;
 *     }
 ...
 * }
 *
 * We could with similar ease find the next multiple of 128 in the set that's
 * larger than 12345 (with ex_nsearch_filtered), iterate over just those
 * multiples of 64 that are in the set (with ex_iter_filtered), or determine if
 * the set contains any multiples of 1024 (with ex_empty_filtered).
 *
...
a_attr void								\
a_prefix##insert(a_rbt_type *rbtree, a_type *node) {			\
    a_prefix##path_entry_t path[sizeof(void *) << 4];			\
...
a_prefix##remove(a_rbt_type *rbtree, a_type *node) {			\
	...
	if (pathp->cmp < 0) {						\
		...
		a_summarize(tnode, rbtn_left_get(a_type, a_field,	\
		    tnode), rbtn_right_get(a_type, a_field, tnode));	\
...
a_prefix##wind(a_rbt_type *rbtree,					\
    a_prefix##path_entry_t path[sizeof(void *) << 4], a_type *node) {	\
    a_prefix##path_entry_t *pathp;					\
    path->node = rbtree->rbt_root;					\
    for (pathp = path; pathp->node != node; pathp++) {			\
...
static inline a_type *							\
a_prefix##search_with_filter_bounds(a_rbt_type *rbtree, a_type *node,	\
  bool (*filter_node)(void *, a_type *),				\
  bool (*filter_subtree)(void *, a_type *),				\
  void *filter_ctx,							\
...
a_attr a_type *								\
a_prefix##search_filtered(a_rbt_type *rbtree, a_type *node,		\
  bool (*filter_node)(void *, a_type *),				\
  bool (*filter_subtree)(void *, a_type *),				\
  void *filter_ctx) {							\
    a_type *result = a_prefix##search_with_filter_bounds(rbtree, node,	\
...
a_attr a_type *								\
a_prefix##nsearch_filtered(a_rbt_type *rbtree, a_type *node,		\
  bool (*filter_node)(void *, a_type *),				\
  bool (*filter_subtree)(void *, a_type *),				\
  void *filter_ctx) {							\
    a_type *node_candidate;						\
...
a_attr a_type *								\
a_prefix##psearch_filtered(a_rbt_type *rbtree, a_type *node,		\
  bool (*filter_node)(void *, a_type *),				\
  bool (*filter_subtree)(void *, a_type *),				\
  void *filter_ctx) {							\
    a_type *node_candidate;						\
